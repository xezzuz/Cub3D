...
/*   Updated: 2024/07/31 13:31:28 by nazouz           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	fill_animation_frames(t_game *game)
{
	...
	while (i < NUM_FRAMES)
	{
		...
		game->animation[i++] = mlx_xpm_file_to_image(game->mlx, frame_name, &x, &x);
		free(frame_name);
	}
	return (1);
}
...
{
	mlx_put_image_to_window(game->mlx, game->win, game->animation[game->current_frame], (WIN_WIDTH / 2) - 540, 0);
